...
function ftgPlaneSpan() {
     ...
     */
    this.drawArrays = function( mvMatrix, rt ) {
        var gl = ftg.gl;
        ...
        // Use alpha channel for blitting
        if ( rt != undefined ) rt.bind();
        gl.clearColor( 0.25, 0.0, 0.0, 1.0 );
        gl.clear( gl.COLOR_BUFFER_BIT );
        if ( rt != undefined ) rt.unbind();
        ...
        for ( var i = this.drawOrder.length-1; i >= 0; --i ) {
            ...
            // Blit plane to render target
            if ( rt != undefined ) rt.bind();
            {
                plane.material.bindTextures( [this.layerRT.texture] );
                ftg.drawTextureToViewport( plane.material, this.layerRT.getWidth(), this.layerRT.getHeight() ); 
                //ftg.drawTextureToViewport( plane.material, gl.viewportWidth, gl.viewportHeight ); 
            }
            if ( rt != undefined ) rt.unbind();
        }
    }
    ...
    this.loadPanel = function( url ) {
        ...
        for ( var i = xmlLayers.length-1; i>= 0; --i ) {
            ...
            var layerMaterial = xmlLayers[i].attributes.getNamedItem( "material" );
            plane.material = ftg.mats[layerMaterial.nodeValue];
            if ( plane.material == undefined )
                console.log( "Unknown material referenced: " + layerMaterial );
