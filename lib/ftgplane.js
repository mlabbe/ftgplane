...
function ftgRenderTarget() {
    ...
    this.init = function() {
        ...
        // todo: pass this in
        var hardcodedWidth = 512;
        var hardcodedHeight = 512;

        // Check constants
        var maxSize = gl.getParameter( gl.MAX_RENDERBUFFER_SIZE );
        if ( hardcodedWidth > maxSize || hardcodedHeight > maxSize ) {
        ...
        this.fb.width = hardcodedWidth;  
        this.fb.height = hardcodedHeight;

        // Create RBOs
        this.rbColor = gl.createRenderbuffer();
        gl.bindRenderbuffer( gl.RENDERBUFFER, this.rbColor );
        gl.renderbufferStorage( gl.RENDERBUFFER, gl.RGB8, 
                                this.fb.width, this.fb.height );
